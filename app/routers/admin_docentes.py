...
from app.esquemas.docente import DocenteCreateAdmin, DocenteAdminResponse, DocenteUpdate
from app.servicios.docente_admin import (
    crear_docente_admin,
    listar_docentes_admin,
    listar_docentes_eliminados_admin,
    obtener_docente_admin,
    actualizar_docente_admin,
    eliminar_docente_admin,
    toggle_docente_admin,
    restaurar_docente_admin,
)
...
def crear_docente_route(
    ...
    # ✅ Ahora: si el email existe pero estaba eliminado -> se restaura automáticamente
...
def listar_docentes_route(
    ...
    # ✅ Lista NO eliminados, y opcionalmente filtra por activo
    return listar_docentes_admin(db, activo=activo)


@router.get("/eliminados", response_model=List[DocenteAdminResponse])
def listar_docentes_eliminados_route(
    db: Session = Depends(get_db),
    admin=Depends(requiere_admin),
):
    # ✅ Lista SOLO eliminados (deleted_at != null)
    return listar_docentes_eliminados_admin(db)
...
def toggle_docente_route(
    ...
    ❌ Si tiene alumnos activos asignados, NO permite desactivar.
    """
    return toggle_docente_admin(db, docente_id)


@router.patch("/{docente_id}/restore", response_model=DocenteAdminResponse)
def restaurar_docente_route(
    docente_id: int,
    db: Session = Depends(get_db),
    admin=Depends(requiere_admin),
):
    ...
    """
    return restaurar_docente_admin(db, docente_id)
...
def eliminar_docente_route(
    ...
    """
    Soft delete.
    ❌ Si tiene alumnos activos asignados, NO permite eliminar.
    """
